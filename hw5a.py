...
def ff(Re, rr, CBEQN=False):
    ...
    if(CBEQN):
        ...
        result= fsolve(cb, 0.02)  # use fsolve to find the frction factor using Colebrook equation
        return result[0]
    else:
        return 64/Re
    pass
...
def plotMoody(plotPoint=False, pt=(0,0)):
    ...
    ReValsCB=np.logspace(np.log10(4000.0), np.log10(1.0E8), 200)  # for use with Colebrook equation (i.e., Re in range from 4000 to 10^8)
    ...
    ffLam=np.array([ff(Re, 0.0) for Re in ReValsL]) # use list comprehension for all Re in ReValsL and calling ff
    ffTrans=np.array([ff(Re, 0.0) for Re in ReValsTrans]) # use list comprehension for all Re in ReValsTrans and calling ff
    ...
    plt.ylabel(r"Friction factor $f=\frac{h_f}{\left(\frac{L}{d}\right)\left(\frac{V^2}{2g}\right)}$", fontsize=16)
